...
class AnswerLookupTable():
  ...
  def extract(self, masker_frequency: str, probe_level: int,
              masker_level: str, probe_frequency: float):
    table_key = self.get_table_key(masker_frequency, probe_level, masker_level)
...
def process_example(raw_example: Dict[str, Any]) -> Tuple[Any]:
  ...
  masker_levels = [int(level) for level in raw_example["MaskLevels"]]
  ...
  return (probe_frequency, probe_level, mask_frequencies, masker_levels,
...
def process_evaluations_file(
  ...
  with open(os.path.join(file_path, evaluations_file), "r") as infile:
    ...
    for raw_example in data:
      (probe_frequency, probe_level, masker_frequencies, masker_levels,
      ...
      processed_data[probe_frequency] = {
          ...
          "masker_frequencies": masker_frequencies,
...
def combine_evaluator_data(all_evaluator_data:
                           ...
                           all_masker_frequencies: List[str],
  ...
  for data_identifier, evaluators_data in all_evaluator_data.items():
    ...
    masker_frequencies = [float(f) for f in masker_frequency.split("+")]
    mask_level = [int(l) for l in all_masker_levels]
    probe_level = int(probe_level)
    current_curve_data = {
        "masker_levels": mask_level,
        "probe_frequencies": [],
        "probe_masking": [],
    }
    ...
    for probe_frequency, data in evaluations_per_probe.items():
      ...
      for example in data:
        ...
        masker_frequencies.extend(example["masker_frequencies"])
        masker_levels.extend(example["masker_levels"])
      ...
      assert len(
          set(masker_frequencies)) == 2, "Different maskers in combining."
      assert len(
          set(masker_levels)) == 1, "Different masker levels in combining."
      assert probe_levels[0] == probe_level, "Wrong match of probe levels"
      assert masker_levels[0] == list(set(mask_level))[0], "Wrong match of masker levels {}, {}".format(masker_levels[0], mask_level)
      ...
      combined_data.append({
          ...
          "worker_ids": [],
          "masker_frequencies": list(set(masker_frequencies)),
          "masker_levels": masker_levels[:2],
          "wavfile_identifier": wavfile_identifiers[1]
      })
  ...
  for masker_frequency in all_masker_frequencies:
    for probe_level in all_probe_levels:
      curve_data.append({"masker_frequencies": masker_frequency,
                       "probe_level": probe_level,
...
def prepare_data_modeling(data: List[Dict[str, Any]],
  ...
  for example in data:
    lookup_table.add(example["masker_frequencies"], example["probe_level"],
                     example["masker_levels"], example["probe_frequency"],
                     example)
  test_set = []
  answers_matched = 0
  for i, masker_probe_curves in enumerate(curve_data):
    masker_frequency = str(masker_probe_curves["masker_frequencies"])
    masker_frequency = str([float(f) for f in masker_frequency.split("+")])
    probe_level = int(masker_probe_curves["probe_level"])
    curves = masker_probe_curves["curves"]
    for j, curve in enumerate(curves):
      curve_data[i]["curves"][j]["failed"] = []
      masker_level = str(curve["masker_levels"])
  ...
  data_plotting.plot_masking_patterns_threetone(curve_data,
...
def get_data(data_directory: str, shape: str, masker_frequencies: List[float],
  ...
  for i in range(num_evaluators):
    ...
    for mask_level in masker_levels:
      for probe_level in probe_levels:
        mask_identifier = "{}+{}".format(int(masker_frequencies[0]),
                                         int(masker_frequencies[1]))
        data_identifier = "{}_{}_{}".format(mask_identifier,
        ...
        masker_dir = "mask_{}Hz_{}dB_probe_{}dB_{}".format(
            str(mask_identifier), int(mask_level), int(probe_level),
            shape)
  ...
  combined_data, curve_data = combine_evaluator_data(data_evaluators,
                                                     ["{}+{}".format(int(masker_frequencies[0]),
                                         int(masker_frequencies[1]))],
                                                     masker_levels,
                                                     probe_levels)
  return combined_data, curve_data
...
def main(argv):
  ...
  data, curve_data = get_data(FLAGS.data_path, FLAGS.signal_shape,
                              ...
                              masker_levels,
                              probe_levels)
  with open(os.path.join(FLAGS.output_directory, "ood_test_{}.json".format(FLAGS.signal_shape)), "w") as outfile:

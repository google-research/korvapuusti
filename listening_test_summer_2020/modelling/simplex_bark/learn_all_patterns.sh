...
# declare -a MASK_FREQUENCIES=(2908.0)
declare -a PROBE_LEVELS=(30 60)
# declare -a PROBE_LEVELS=(60)
declare -a MASKING_LEVELS=(40 60 80)
# declare -a MASKING_LEVELS=(60)
echo "Mask_freq, Probe_level, Mask_level, Loss, Var0, Var1, Var2" &> all_vars_bark_sk.txt
for MASK_FREQ in "${MASK_FREQUENCIES[@]}"; do
  ...
  for PROBE_LEVEL in "${PROBE_LEVELS[@]}"; do
    ...
    for l in "${MASKING_LEVELS[@]}"; do
      ...
      ./simplex_fork.py ./loss.py 3 0.01 500 &> logs/${MASK_FREQ}_${PROBE_LEVEL}_${l}/${MASK_FREQ}_${PROBE_LEVEL}_${l}.txt
      ./get_logs.sh logs/${MASK_FREQ}_${PROBE_LEVEL}_${l}/${MASK_FREQ}_${PROBE_LEVEL}_${l}.txt logs/${MASK_FREQ}_${PROBE_LEVEL}_${l}/logs.txt
      bestVars=`cut -f1,2,3,4 -d"," logs/${MASK_FREQ}_${PROBE_LEVEL}_${l}/logs.txt | sort -n | head -1`
      echo "${MASK_FREQ}, ${PROBE_LEVEL}, ${l}, ${bestVars}" >> all_vars_bark_sk.txt
